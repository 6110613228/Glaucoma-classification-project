...
def get_CD_r(image):
    """
    Extract Disc radius, Cup radius
    @param image
    @return [Disc radius, Cup radius]
    """
    gaus = signal.gaussian(60, std=7)
    ...
    rhist, rbins = np.histogram(pre_r, 256, [0,256])
    ghist, gbins = np.histogram(pre_g, 256, [0,256])

    # Smoothen preprocessed graph
    ...
    # Threshold for Disc segmentation
    T1 = preprocess_r.std()
    ...
    disc = cv.morphologyEx(disc, cv.MORPH_CLOSE, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (5, 5)))
    disc = cv.morphologyEx(disc, cv.MORPH_OPEN, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (5, 5)))
    disc = cv.morphologyEx(disc, cv.MORPH_CLOSE, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (11, 11)))
    disc = cv.morphologyEx(disc, cv.MORPH_OPEN, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (11, 11)))
    disc = cv.morphologyEx(disc, cv.MORPH_CLOSE, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (21, 21)))

    preprocess_g = np.array(preprocess_g)
    # Threshold for Cup segmentation
    T2 = 2 * preprocess_g.std()
    ...
    cup = cv.morphologyEx(cup, cv.MORPH_CLOSE, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (5, 5)))
    cup = cv.morphologyEx(cup, cv.MORPH_OPEN, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (5, 5)))
    cup = cv.morphologyEx(cup, cv.MORPH_CLOSE, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (11, 11)))
    cup = cv.morphologyEx(cup, cv.MORPH_OPEN, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (11, 11)))
    cup = cv.morphologyEx(cup, cv.MORPH_CLOSE, kernel=cv.getStructuringElement(cv.MORPH_ELLIPSE, (21, 21)))

    #Getting all possible contours in the segmented image
    contours = cv.findContours(disc, cv.RETR_EXTERNAL,cv.CHAIN_APPROX_SIMPLE)[0] 
    largest_area = 0
    disc_circle = contours[0]
    if len(contours) != 0:
        for i in range(len(contours)):
            if len(contours[i]) >= 5:
                area = cv.contourArea(contours[i]) #Getting the contour with the largest area
    ...
    #Getting all possible contours in the segmented image
    contours = cv.findContours(cup, cv.RETR_EXTERNAL,cv.CHAIN_APPROX_SIMPLE)[0] 
    largest_area = 0
    ...
    # get_CD_r return Cup, Disc radius
    return [disc_r, cup_r]

def get_exudate(img):

    fundus = cv.resize(img,(800,615))
    fundus_mask = cv.imread("MASK.bmp")
    ...
    newfin = cv.dilate(edge_feature_output, cv.getStructuringElement(cv.MORPH_ELLIPSE,(5,5)), iterations=1)
    edge_candidates = cv.erode(newfin, cv.getStructuringElement(cv.MORPH_ELLIPSE,(3,3)), iterations=1)
...
def edge_pixel_image(image,bv_image):
    ...
    while i < image.shape[0]:
        ...
        while j < image.shape[1]:
            ...
            j = j+1
        i = i+1
    newfin = cv.dilate(edge_result, cv.getStructuringElement(cv.MORPH_ELLIPSE,(3,3)), iterations=1)
    return newfin
...
def extract_bv(image):			
    ...
    contours, hierarchy = cv.findContours(f6.copy(),cv.RETR_LIST,cv.CHAIN_APPROX_SIMPLE)
    for cnt in contours:
    ...
    dilated = cv.erode(blood_vessels, cv.getStructuringElement(cv.MORPH_ELLIPSE,(7,7)), iterations=1)
